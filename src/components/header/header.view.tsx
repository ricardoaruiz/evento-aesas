...
export function Header({ title, subtitle, color, className }: BannerProps) {
  return (
    <header
      className={cn(
        "flex flex-col justify-between gap-4 bg-white px-10 py-10 text-black md:flex-row",
        "bg-[url('/images/fundo.png')] bg-cover bg-center",
        className,
      )}
      style={{ color }}
    >
      <div className="flex flex-col items-center gap-8 p-2 md:items-start">
        <h1 className="font-biryani text-center text-4xl font-bold md:text-left md:text-5xl lg:text-6xl">
          {title}
        </h1>
        <h2 className="font-glacial text-center text-2xl font-semibold md:text-left md:text-3xl lg:text-4xl">
          {subtitle}
        </h2>
      </div>

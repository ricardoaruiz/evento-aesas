import { Card } from "./card/card.view";

export function Cards() {
  return (
    <div className="container mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1">
      <Card
        title="SETEMBRO"
        contentTitle="INTRUSÃO DE VAPORES"
        contentDescription="aulas on-line"
        onlineTime="das 19h às 22h"
        onlineDates="01/09, 03/09, 08/09, 10/09, 15/09, 17/09, 22/09, 24/09, 29/09"
        footerText="Carga Horária: 27h"
        tagText="NOTURNO"
        tagTextSize="large"
        tagVariant="silver"
        // className="w-[220px] min-h-[300px]"
      />
      <Card
        title="SETEMBRO"
        contentTitle="INVESTIGAÇÃO DETALHADA"
        contentDescription="aulas on-line"
        onlineTime="das 8h às 12h"
        onlineDates="02/09, 04/09, 09/09, 11/09, 16/09, 18/09"
        footerText="Carga Horária: 24h"
        // className="w-[220px] min-h-[300px]"
      />
      <Card
        title="OUTUBRO"
        contentTitle="CLASSIFICAÇÃO DE RESÍDUOS (NBR 10.004)"
        contentDescription="aulas on-line"
        onlineTime="das 8h às 12h"
        onlineDates="02/09, 04/09, 09/09, 11/09, 16/09, 18/09"
        footerText="Carga Horária: 24h"
        tagText="NOTURNO"
        tagTextSize="large"
        tagVariant="silver"
        // className="w-[220px] min-h-[300px]"
      />
      <Card
        title="OUTUBRO"
        contentTitle="INTRODUÇÃO  À REMEDIAÇÃO"
        contentDescription="aulas on-line"
        onlineTime="das 8h às 12h"
        onlineDates="02/09, 04/09, 09/09, 11/09, 16/09, 18/09"
        footerText="Carga Horária: 24h"
        tagText="EXCLUSIVO OEMAS"
        tagTextSize="small"
        tagVariant="gold"
        // className="w-[220px] min-h-[300px]"
      />
      <Card
        title="NOVEMBRO"
        contentTitle="GERENCIAMENTO DE PROJETOS NO GAC"
        contentDescription="aulas on-line"
        onlineTime="das 8h às 12h"
        onlineDates="02/09, 04/09, 09/09, 11/09, 16/09, 18/09"
        footerText="Carga Horária: 24h"
        tagText="NOTURNO"
        tagTextSize="large"
        tagVariant="silver"
        // className="w-[220px] min-h-[300px]"
      />
      <Card
        title="NOVEMBRO"
        contentTitle="GEOLOGIA E HIDROGEOLOGIA APLICADA AO GAC"
        contentTileSize="small"
        contentDescription="aulas on-line"
        onlineTime="das 8h às 12h"
        onlineDates="02/09, 04/09, 09/09, 11/09, 16/09, 18/09"
        presencialDates="02/09, 04/09, 09/09, 11/09, 16/09, 18/09"
        presencialTime="das 13h às 17h"
        footerText="Carga Horária: 24h"
        // className="w-[220px] min-h-[300px]"
      />
    </div>
  );
}
